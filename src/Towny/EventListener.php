...
class EventListener implements Listener
{
    ...
    {
        if (($towny = TownyLoader::getInstance()->getTownyFactory()->getTownyByXZ((float) $player->getX(),
                                                                                  (float) $player->getZ(),
                                                                                  $player->getPosition()->getWorld()->getFolderName())) instanceof Towny) {
            ...
            try {
                ...
                for ($x = $block->getPos()->getX() - (int) $defaultSize; $x <= $block->getPos()->getX() + (int) $defaultSize; $x++) {
                    for ($z = $block->getPos()->getZ() - (int) $defaultSize; $z <= $block->getPos()->getZ() + (int) $defaultSize; $z++) {
                        if (TownyLoader::getInstance()->getTownyFactory()->getTownyByXZ((float) $x,
                                                                                        (float) $z,
                                                                                        $block->getPos()->getWorld()->getFolderName()) instanceof Towny) {
                            $player->sendMessage(TownyLoader::$prefix .
                                                 TownyLoader::getInstance()->getLanguage()->translateString("towny.message.create.overlapPos", [0 => $defaultSize]));
                            break;
                        }
                    }
                }
                ...
                $towny = new Towny(TownyLoader::getInstance(),
                                   $name,
                                   $start,
                                   $end,
                                   $block->getPos(),
                                   [],
                                   new TownyOption(false, false),
